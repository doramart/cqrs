...
Object.defineProperties(AggregateRoot.prototype, {
    ...
    _apply: {
        value: function (name, data,sagaId) {
            if (this.get("alive")) {
...
AggregateRoot.extend = function (option) {
    ...
    Class.className = option.className;

    inherits(Class, AggregateRoot);

    function buildMethod(name,method){
        Class.prototype[name] = function(data,_sagaId){
            var self = this;
            function apply(name,data){
                self._apply(name,data,_sagaId);
            }
            method.call(this,data,apply);
        }
    }

    for (var k in option.methods || {}) {
        buildMethod(k,option.methods[k]);
    }
